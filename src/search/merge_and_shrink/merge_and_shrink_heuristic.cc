...
MergeAndShrinkHeuristic::MergeAndShrinkHeuristic(const Options &opts)
    : Heuristic(opts),
      ...
      bound(opts.contains("bound") ? opts.get<int>("bound") : INF),
...
void MergeAndShrinkHeuristic::warn_on_unusual_options() const {
    ...
    if(bound < 0) {
      cerr << dashes << endl
	   << "WARNING! Cost-Bound is negative!"
	   << endl << dashes << endl;
    }
}
...
void MergeAndShrinkHeuristic::build(const utils::Timer &timer) {
    ...
    if(bound < INF) {
      final_entry.second->compute_backward_pareto_fronts(verbosity);
    }
  ...
  int MergeAndShrinkHeuristic::compute_heuristic(const GlobalState &global_state, int g, int bound, int u) {
    (void)u;
    State state = convert_global_state(global_state);
    int h = mas_representation->get_value(state, g, bound);
    if (h == PRUNED_STATE)
      h = DEAD_END;
