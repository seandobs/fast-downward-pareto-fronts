...
public:
  ...
  FactoredTransitionSystem create(Verbosity verbosity, bool finalize_if_unsolvable, int bound);
};
...
vector<unique_ptr<Distances>> FTSFactory::create_distances(
  ...
  vector<unique_ptr<Distances>> FTSFactory::create_distances(
    ...
    for (int var_no = 0; var_no < num_variables; ++var_no) {
        result.push_back(
			 utils::make_unique_ptr<Distances>(*transition_systems[var_no], bound));
    }
    return result;
}
...
FactoredTransitionSystem FTSFactory::create(
  ...
  FactoredTransitionSystem FTSFactory::create(
        Verbosity verbosity, bool finalize_if_unsolvable, int bound) {
    if (verbosity >= Verbosity::NORMAL) {
        cout << "Building atomic transition systems... " << endl;
    }
    ...
    vector<unique_ptr<MergeAndShrinkRepresentation>> mas_representations =
        create_mas_representations();
    vector<unique_ptr<Distances>> distances =
      create_distances(transition_systems, bound);
...
FactoredTransitionSystem create_factored_transition_system(
  ...
  FactoredTransitionSystem create_factored_transition_system(
    ...
    int bound,
    bool finalize_if_unsolvable) {
    return FTSFactory(task_proxy).create(verbosity, finalize_if_unsolvable, bound);
}
}
